...
- **Admin API:** `POST /api/stripe-sync` with actions: `bulkSync`, `clearAndResync`, `getSyncStatus`, `getDiscountStatus`, `endDiscount`, `listDiscountMembers`, `applyDiscount`, `getResyncImpact` (all admin-only)
...
- **Service:** `app/services/stripe-discounts.server.ts`
  ...
  - `resolveDiscountCode(code)` — resolves an admin-typed promotion code (or a bare coupon id) to the coupon behind it, reporting the products it is limited to and any promotion-code restrictions. Always reads the coupon with `expand: ["applies_to"]`, because a plain retrieve omits that field entirely and every scoped coupon would otherwise read as unscoped
  - `getCouponDetails(couponIds)` — reads several coupons at once, deduplicated, returning product scope, amount, duration and the human-facing promotion codes pointing at each. One Stripe call per distinct coupon rather than per member; a coupon that cannot be read is omitted, so callers treat missing as unknown rather than as unscoped
  - `applyMembershipDiscount(customerId, couponId, userMembershipId?, promotionCodeId?)` — pins the coupon to the customer and mirrors it locally. Passing `promotionCodeId` redeems the promotion code instead of the bare coupon, which is what makes Stripe enforce the limits set on the code
...
- **Applying one to an existing member:** Admin Settings → Stripe Integrations → **Apply a discount to a member** takes an active member and a Stripe promotion code, and pins it the same way checkout would. `listDiscountEligibleMembers()` fills the picker (status `active` only, the one state the billing cron charges again) and `applyRecurringDiscountToMember(userId, code)` does the work. It takes effect from the member's next renewal; nothing already paid is refunded
- **What it refuses, before touching Stripe:** a code Stripe does not know, a coupon Stripe reports as no longer valid, a promotion code carrying a `minimum_amount` or `first_time_transaction` restriction (Stripe will not attach those to a Customer at all), and a coupon limited to products that do not include the member's plan. That last one matters most: Stripe ignores a product-scoped coupon on an invoice line for a different product, charging full price while reporting success, so the check refuses rather than letting it apply silently
- **When a discount ends:** a discount never outlives the membership it was granted for. Two separate rules drive this:
  - **The plan changed:** upgrade or downgrade ends it unconditionally (`endRecurringDiscountForUser()`), because the discount was granted for the old plan
  - **The membership ended:** every exit calls `endDiscountForEndedMembership()`, which ends it *unless a membership the member still holds can actually use the coupon*. Surviving a membership is not enough on its own: an unscoped coupon covers anything they keep, but one scoped to the plan that just ended is dormant against everything else, discounting nothing, invisible in the admin list, and liable to spring back if they rejoin the old plan. A coupon Stripe cannot be read for is left alone, because destroying a working discount is the worse mistake. The guard matters because the coupon lives on the Stripe customer, and an `ending` row beside an `active` row is the normal shape right after an upgrade
- **Every exit is covered**, because they do not all pass through `inactive`: the three cron branches (auto-renew off, no usable card, `ending`/`cancelled` reaching term), **cancelling after the cycle has ended** (which deletes the row outright rather than retiring it), and **an admin revoke** (which sets `revoked` and is never undone, since `unrevokeUserMembershipByAdmin()` restores only the user-level block, not the membership rows). Cancelling *mid-term* deliberately keeps the discount, because they are still a member until the term ends
- An admin can also end one manually with **End** in Admin Settings → Stripe Integrations, and Stripe ends it on the coupon's own expiry
- **Clear & Re-sync destroys product-scoped discounts, so it asks first.** `bulkSyncToStripe(true)` gives every plan a *new* Stripe Product, and a coupon's `applies_to.products` is fixed at creation and cannot be re-pointed, so any coupon limited to specific products can never apply again. Before running, the button calls `listProductScopedDiscounts()` and shows exactly who is affected, with a CSV worklist carrying step-by-step restore instructions. On confirm, `clearOrphanedScopedDiscounts()` ends those discounts and writes the full list to `logs/all_logs.log` at `info` level, so the record survives even if the download is never taken. The post-run panel offers the CSV again, now naming each plan's **new** product id to scope the replacement coupon to
- **Unscoped coupons are unaffected** by a re-sync and are deliberately left alone: with no product limit they match whatever the renewal invoice carries. Since customer discounts apply to invoices only, and membership renewals are the only invoices this app raises, an unscoped coupon cannot leak into workshop or equipment sales either. Scope a coupon only when the discount should apply to some plans and not others
...
- `endRecurringDiscountForUser()` — removes a member's recurring discount in Stripe and clears the mirrored columns; called on upgrade, downgrade, an admin revoke, and the admin **End** action
- `endDiscountForEndedMembership(userId, endedMembershipId)` — the membership-ended rule: ends the discount unless a membership the member still holds can actually use the coupon. An unscoped coupon survives, one scoped to the plan that just ended does not, and a coupon Stripe cannot be read for is left alone
- `applyRecurringDiscountToMember(userId, code, membershipId?)` — the mirror image: gives an existing active member a recurring discount from an admin-typed promotion code. Validates everything before touching Stripe, so a refusal leaves nothing half-applied; returns a typed `reason` and a message ready to show an admin. Pass `membershipId` when the member holds more than one active membership, so the product check runs against the plan the admin actually chose
- `listDiscountEligibleMembers()` — active members with plan, billing cycle, next payment date, current coupon, and whether they have a saved card; fills the admin picker, one row per active membership
- `listProductScopedDiscounts()` — members whose product-scoped coupon a Clear & Re-sync would destroy, with the coupon's terms and promotion code; unscoped coupons are excluded because they survive
- `clearOrphanedScopedDiscounts(affected)` — ends those discounts and logs the worklist to `logs/all_logs.log` first, so the record survives even if the download is never taken
...
The suite is currently **fully green — 46 suites, 737 tests** — and every server module is covered. That baseline is what makes step 4 meaningful: a failure after your change is a regression you introduced, not background noise. Do not commit on a red suite without saying so explicitly.
