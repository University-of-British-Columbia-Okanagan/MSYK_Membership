...
export default function MembershipPlanForm({ mode, form, defaultValues, submitLabel = mode === "create" ? "Submit" : "Confirm", initialShowMultipleBilling = false, }: Props) {
  const [features, setFeatures] = useState<string[]>(defaultValues.features || [""]);
  const [showMultipleBilling, setShowMultipleBilling] = useState<boolean>(initialShowMultipleBilling);
  const [needAdminPermission, setNeedAdminPermission] = useState<boolean>(Boolean((defaultValues as any).needAdminPermission));

  useEffect(() => {
    setFeatures(defaultValues.features && defaultValues.features.length > 0 ? defaultValues.features : [""]);
  ...
  }, [defaultValues, initialShowMultipleBilling]);
  ...
  return (
    <Form {...form}>
      <form method="post">
        ...
        <div className="space-y-4 mt-4 mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <h3 className="text-sm font-medium text-gray-900">Add Multiple Billing Options</h3>
            </div>
            <button
              type="button"
          ...
          {showMultipleBilling && (
            <div className="space-y-4 pt-4 border-t border-gray-300">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-2">Quarterly Plan Price</FormLabel>
                    <FormControl>
                    ...
                    <p className="text-xs text-gray-500">Leave empty if you don't want to offer quarterly billing</p>
                    <FormMessage />
                  </FormItem>
                )}
              />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-2">Semi-annual Plan Price</FormLabel>
                    <FormControl>
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">CA$</span>
                        <Input
                    ...
                    <p className="text-xs text-gray-500">Leave empty if you don't want to offer semi-annual billing</p>
                    <FormMessage />
                  </FormItem>
                )}
              />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-2">Yearly Plan Price</FormLabel>
                    <FormControl>
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">CA$</span>
                        <Input
                    ...
                    <p className="text-xs text-gray-500">Leave empty if you don't want to offer yearly billing</p>
                    <FormMessage />
                  </FormItem>
                )}
              />
        ...
        <div className="p-4 mt-4 mb-4 bg-gradient-to-br from-gray-50 to-slate-50 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex-1 mr-4">
              <h3 className="text-sm font-medium text-gray-900">Require Admin Permission</h3>
        ...
        <input type="hidden" name="needAdminPermission" value={needAdminPermission ? "true" : "false"} />
