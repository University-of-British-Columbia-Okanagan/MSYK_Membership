...
export async function action({ request, params }: { request: Request; params: { id: string } }) {
    ...
    if (currentUserRole?.roleName !== "Admin") {
        logger.warn(`[User: ${currentUserRole?.userId ?? "unknown"}] Not authorized to delete equipment`, {
            url: request.url,
            });
    ...
    try {
        ...
        logger.info(`[User: ${currentUserRole?.userId ?? "unknown"}] Equipment ${equipmentId} deleted successfully`, {
            url: request.url,
            });
        return { success: true };
    } catch (error) {
