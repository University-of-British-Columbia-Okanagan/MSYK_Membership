...
- **Paying in a flow?** Use Stripe's test card `4242 4242 4242 4242` (any future expiry, any CVC, any non-empty name/email/billing address) on test keys only. Note `QuickCheckout` appears only when the user already has a saved payment method (`UserPaymentInformation`) — `npx tsx seed.ts` wipes saved cards along with the user rows, via cascade, so re-add one at `/user/profile/paymentinformation` → Add Payment Method. Details in `tests/README.md`
...
- **Ask if you are blocked.** If driving the flow needs something you do not have — a sandbox credential, a record in a particular state, a decision about what should happen — ask for it rather than skipping the verification
