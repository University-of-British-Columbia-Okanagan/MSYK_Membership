...
| AC16 | **Membershi** Cancellation (Before Cycle End) | Cancel membership before `nextPaymentDate` | Membership status "cancelled"; user retains access until `nextPaymentDate`; role level unchanged; cancellation confirmation email received |
...
| ---- | **Workshops:** Route UI Showcase for Users and Guests (/dashboard/workshops) | No add workshop button and cannot access dashboard/addworkshop route. No edit or delete workshop buttons and cannot access those routes on the workshop card or through URL. Can see default workshop picture unless otherwise has an image |
| ---- | **Workshop:** Route UI Showcase for Admins (/dashboard/workshops or /dashboard/admin) | Can access add workshop button and can access dashboard/addworkshop route. Has edit or delete add workshop buttons on each workshop card. Can see default workshop picture unless otherwise has an image |
...
| ---- | **Equipments:** Route UI Showcase for Users and Guests | No add equipment button and cannot access dashboard/addequipments route. No edit or delete equipment buttons. Unable to access unavaliable equipments with proper UI. Can see default equipment picture unless otherwise has an image |
...
- **Findings**:
  ...
  - Workshop that are in the register cut-off phase can still be accessed and registered by typing URL: http://localhost:5173/dashboard/payment/:workshopID/:workshopOccurrenceID for single occurrence and http://localhost:5173/dashboard/payment/:workshopID/connect/:connectID for multi-day workshops
   ...
   - When people used to have an membership but it is now inactive, sometimes, they cannot subscribe to another because it says "You can change only after your old membership billing cycle ends." An example of when this can happen is when it is Nov 21st for example but in the db in UserMembership, startDate is 2025-11-12 03:35:51.909 and nextPaymentDate is 2025-12-12 03:35:51.909
   - We should probably disable the ability to edit equipment prereqs after creating them. Implement this
   - In /myequipments, bookings in the past should not show up
